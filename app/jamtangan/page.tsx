// src/app/celana/page.tsx (atau folder jam-tangan kamu)
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {products.map((item) => (
          <div key={item.id} className="border p-4 rounded-lg shadow hover:shadow-lg transition bg-white">
            <div className="relative w-full h-48 mb-4">
               <img src={item.thumbnail} alt={item.title} className="w-full h-full object-contain"/>
            </div>
            <h2 className="font-bold text-lg">{item.title}</h2>
            <p className="text-red-600 font-bold text-xl mt-2">Rp {item.price * 15000}</p> {/* Pura-pura Rupiah */}
          </div>
        ))}
      </div>
    </div>
